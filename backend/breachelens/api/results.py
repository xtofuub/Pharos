...
from breachelens.errors import BadRequestError, NotFoundError, PathNotAllowedError
...
def _numeric_record_id(record_id: str) -> int:
    ...
    if not value.isdigit():
        raise BadRequestError("invalid record id")
    return int(value)
...
async def reveal(
    ...
    row = await state.db.fetchone(
        """
        SELECT r.id, r.source_id, r.file_id, r.file_path, r.line_number,
               r.byte_offset, r.byte_length, f.size_bytes, f.mtime
        FROM records r
    ...
    try:
        ...
        raise NotFoundError("source file no longer exists")
    source = await state.db.fetchone("SELECT path FROM sources WHERE id = ?", (row["source_id"],))
    if source is None or not is_within(canonical, Path(source["path"])):
        raise PathNotAllowedError("record file is outside its indexed source")
    stat = canonical.stat()
    if row["mtime"] is not None and int(stat.st_mtime) != int(row["mtime"]):
        raise BadRequestError("source file changed after indexing; re-index it before revealing")
    if row["size_bytes"] is not None and stat.st_size != int(row["size_bytes"]):
        raise BadRequestError("source file size changed after indexing; re-index it before revealing")
    max_read = min(state.config.indexing.max_line_length * 4, 1024 * 1024)
    byte_length = max(0, min(int(row["byte_length"]), max_read))
    if byte_length == 0:
        raise BadRequestError("record has an invalid byte length")
